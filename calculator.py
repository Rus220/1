...
def _get_car_age_category(year: int, current_year: int = 2026, *,
                          month: int = 0, current_month: int = 0) -> str:
    """Determine age category. When *month* and *current_month* are provided
    (1-12), the comparison is precise to the month."""
    if month >= 1 and current_month >= 1:
...
def _get_util_fee(year: int, hp: Decimal, engine_cc: int, engine_type: str,
                  ...
                  month: int = 0, current_month: int = 0) -> tuple[Decimal, str]:
    if month >= 1 and current_month >= 1:
        total_months = (current_year - year) * 12 + (current_month - month)
        if total_months < 0:
            total_months = 0
    ...
    table = UTIL_FULL if age_years <= 3 else UTIL_FULL_OLD
    for max_cc, fee in table:
        if engine_cc <= max_cc:
            label = "до 3 лет" if age_years <= 3 else "старше 3 лет"
...
def calculate(
    ...
    *,
    ...
    age_cat = _get_car_age_category(year, current_year,
                                    month=month, current_month=current_month)
    if age_cat == "0-3":
